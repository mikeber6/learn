...
raiserror('Beginning install at %s ....',1,1,@dttm) with nowait

-- create the database

if exists (SELECT dbid FROM sysdatabases WHERE name = 'OfficeSupply')
begin
  raiserror('Dropping existing database ....',0,1)
  DROP database OfficeSupply 
end
GO
raiserror('Creating OfficeSupply database....',0,1)
CREATE DATABASE OfficeSupply;
GO
ALTER DATABASE OfficeSupply 
...
INSERT INTO Product VALUES ('OIC-5000  ',2,'Glue Stick','Non toxic',1.99,2);
...
raiserror('Creating stored procedures....',0,1)
go
-------------------------------------------------------------------------------
-- up_PlaceOrder
-------------------------------------------------------------------------------
